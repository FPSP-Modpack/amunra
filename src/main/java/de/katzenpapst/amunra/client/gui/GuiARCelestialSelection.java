...
public class GuiARCelestialSelection extends GuiCelestialSelection {
    ...
    public void initGui() {
        ...
        for (Mothership ms : msData.getMotherships()
            .values()) {
    ...
    protected boolean isSiblingOf(CelestialBody celestialBody, Mothership ship) {
        if (celestialBody instanceof Planet) {
            ...
            return solarSystem.getMainStar()
                .equals(ship.getParent());
            ...
            ((Mothership) celestialBody).getParent()
                .equals(ship.getParent());
        }
                ...
                if ((ms == this.selectedBody || (ms.getParent() == this.selectedBody && this.selectionCount != 1))
                    && (this.ticksSinceSelection > 35 || this.selectedBody == ms
                        || (this.lastSelectedBody instanceof Mothership
                            && ((Mothership) this.lastSelectedBody).getParent()
                                .equals(ms)
    ...
    public static int getWidthForMothershipStatic(Mothership celestialBody) {
         ...
         * }
         * if()
         * return celestialBody instanceof Star ? 8 : (celestialBody instanceof Planet ? 4 : (celestialBody instanceof
         * IChildBody ? 4 : (celestialBody instanceof Satellite ? 4 : 2)));
         * return celestialBody instanceof Star ? 12 : (celestialBody instanceof Planet ? 6 : (celestialBody instanceof
         * IChildBody ? 6 : (celestialBody instanceof Satellite ? 6 : 2)));
         */
        // return 2;
    }
    ...
    public void mothershipListUpdated() {
        ...
        for (Iterator<Entry<CelestialBody, Integer>> it = this.celestialBodyTicks.entrySet()
            .iterator(); it.hasNext();) {
    ...
    protected Vector2f getTranslationAdvanced(float partialTicks) {
         ...
         * this.lastSelectedBody).getParentPlanet() == this.selectedBody)
         * {
