...
public class GuiHydroponics extends GuiContainerGC {
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        if (growStatus < 0) {
            ...
            String status = GCCoreUtil.translate("gui.status.collecting.name") + ": "
                + (int) (0.5F
                    + Math.min(this.tile.lastOxygenCollected * 20F, TileEntityHydroponics.OUTPUT_PER_TICK * 20F))
    ...
    private String getStatus() {
         ...
         * this.tile.lastOxygenCollected <= 0.0F)
         * {
        ...
        if (this.tile != null) {
            ...
            oxygenDesc.add(
                EnumColor.YELLOW + GCCoreUtil.translate("gui.oxygenStorage.desc.1")
                    + ": "
                    + ((int) Math.floor(this.tile.storedOxygen) + " / " + (int) Math.floor(this.tile.maxOxygen)));
