...
public abstract class TileEntityMothershipEngineAbstract extends TileBaseElectricBlockWithInventory
    ...
    protected void processFluids() {
        ...
        if (canister != null) {
             ...
             * int used = this.fuelTank.fill(new FluidStack(GalacticraftCore.fluidFuel, ItemCanisterGeneric.EMPTY -
             * originalDamage), true);
             ...
             * if (FluidContainerRegistry.isBucket(this.containingItems[0]) &&
             * FluidContainerRegistry.isFilledContainer(this.containingItems[0]))
             * {
             * final int amount = this.containingItems[0].stackSize;
             * if (amount > 1) {
             * this.fuelTank.fill(new FluidStack(GalacticraftCore.fluidFuel, (amount - 1) *
    ...
    public boolean canFill(ForgeDirection from, Fluid fluid) {
        ...
        if (CoordHelper.rotateForgeDirection(ForgeDirection.NORTH, metadata)
            .equals(from)) {
            return false;
        }
        return true;
    ...
    protected boolean attachBooster(int x, int y, int z, boolean notifyClient) {
        ...
        if (!booster.getBlock()
            .equals(worldBlock) || booster.getMetadata() != worldMeta
            || worldTile == null
